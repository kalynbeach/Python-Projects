# Multi-Conversion Manager by Kalyn Beach
...
# Manage choice by switching to appropriate conversion function:
...
# Display menu and implement user choice repeatedly:

def repeat():
    displayMenu();
    currentChoice = input('Enter choice number: ');
...
# Display menu and implement user choice with a pretest loop (same as repeat):
...
# Display menu and implement user choice with an interrupted loop:

def repeatInterrupted():
    while (True):
        displayMenu();
        currentChoice = input('Enter choice number: ');
        if (currentChoice == 0):
